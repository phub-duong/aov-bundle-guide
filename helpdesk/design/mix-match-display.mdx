---
title: "Mix-and-Match Display Settings"
description: "Customize the appearance of the Mix-and-Match Bundle widget on your storefront in AOV Bundle."
---

import { Steps, Callout, Cards } from 'nextra/components'

# Mix-and-Match Display Settings

Customize how the Mix-and-Match Bundle widget looks on your storefront. Go to **AOV.ai Bundle Discounts > Design > Mix-and-Match Bundle** to access these settings.

<Callout type="info">
  **Prerequisites:** AOV Bundle installed with at least one Mix-and-Match offer created.
</Callout>

## Steps

<Steps>

### Step 1: Configure widget content

1. **Select layout widget**
   - Choose the layout that best fits your bundle page design.

2. **Edit content text**
   - **Title**: The main headline on the bundle page. Default: "Design your own ideal pack!"
   - **Description**: A subtitle. Default: "Grab what catches your eye, and we'll sweeten the deal just for you!"
   - **Total products label**: Shows the total selected. Default: "Total products:"
   - **Save badge label**: Shows the discount. Uses `{{discount_value}}` variable.
   - **Promotion message**, **Process message**, **Promotion reached message**: Messages shown during the bundle selection process.

3. **Translation** — Click **Translation** to add text in other languages. Keep variables like `{{discount_value}}` intact.

![Widget content settings](/images/mix-match-display/display-step-1.png)

### Step 2: Theme and content settings

Click **Edit widget** to access the full design editor.

#### Theme setting

This tab controls the **visual style** of the Mix-and-Match Bundle widget.

1. **Color styles** — Choose between:
   - **Basic** — Pick a primary and secondary color for automatic styling.
   - **Advanced** — Set colors individually:
     - **General**: Primary text, secondary text, price color.
     - **Option**: Border unselected, selected border, background quantity, quantity text.
     - **Badge discount**: Background and text colors.
2. **Themes** — Choose a preset color theme from the available options.

![Theme settings](/images/mix-match-display/display-step-2a.png)

#### Content setting

This tab controls the **text labels** inside the widget.

1. **Widget text labels** — Customize:
   - Option names and descriptions.
   - Bundle summary labels.
   - Button text and badge text.
2. **Translations** — Click **Translation**, select a language, and translate each label. Keep variables intact.

![Content settings](/images/mix-match-display/display-step-2b.png)

### Step 3: Preview and save

Look at the **live preview panel on the right** to see your changes. When satisfied, click **Save** to apply your settings.

![Preview and save](/images/mix-match-display/display-step-3.png)

</Steps>

Your Mix-and-Match Bundle widget is now customized and ready for customers to build their own bundles!

## What's Next

<Cards>
  <Cards.Card
    title="Create a Mix-and-Match Bundle"
    href="/docs/offers/create-mix-match"
    arrow
  />
  <Cards.Card
    title="Customize Widget Appearance"
    href="/docs/design/customize-widget"
    arrow
  />
  <Cards.Card
    title="Auto-Fit to Your Store Theme"
    href="/docs/design/theme-auto-fit"
    arrow
  />
</Cards>

---

*Was this article helpful? [Let us know](https://github.com/anthropics/claude-code/issues).*
